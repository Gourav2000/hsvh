            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Spacer(),
                    Text('Did not Recieve Code ?',style: TextStyle(color: Colors.blue),)
                  ],
                ),
                ...
                onTap: ()async{
                  if(textEditingController.text.length!=6)
                    errorController.add(ErrorAnimationType.shake);
                      ...
                      final res=await AuthServices.instance.verifyOtp(widget.phnNumber, textEditingController.text);
                      if(res=='new User')
                        {
                          print('new User');
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>RegisterPage(phnNumber: widget.phnNumber,countryDialCode: widget.countryDialCode,countryCode: widget.countryDialCode,)));
                      ...
                      else if(res=='old User')
                        {
                          ...
                          if(FirebaseAuth.instance.currentUser!.displayName!='' && FirebaseAuth.instance.currentUser!.displayName!=null)
                            ...
                            PhoneNumber phoneNumber = await PhoneNumberUtil().parse(FirebaseAuth.instance.currentUser!.phoneNumber.toString());
                            print('zz= ${FirebaseAuth.instance.currentUser!.phoneNumber.toString()}');
                            Navigator.push(context, MaterialPageRoute(builder: (context)=>RegisterPage(email: FirebaseAuth.instance.currentUser!.email,phnNumber: phoneNumber.nationalNumber,countryDialCode: '+${phoneNumber.countryCode}',)));
                          }
                        }
                      else{
                        errorController.add(ErrorAnimationType.shake);
